...
export function HandbookIntake({ canManageSource }: { canManageSource: boolean }) {
  ...
  return <div className="mx-auto flex w-full max-w-5xl flex-col gap-7 py-4">
    ...
    {!canManageSource && <p className="border-l-2 border-border pl-3 text-sm text-muted">This account may inspect verified source material but cannot upload or review it.</p>}

    <section className="grid gap-4 border-b border-border pb-6 md:grid-cols-[minmax(0,1fr)_220px_auto] md:items-end">
      <label className="grid gap-2 text-sm font-medium">
        Handbook PDF
        <input ref={inputRef} aria-label="Handbook PDF" type="file" accept="application/pdf,.pdf" disabled={!canManageSource} onChange={selectFile} className="block w-full text-sm text-muted file:mr-3 file:rounded file:border-0 file:bg-accent file:px-3 file:py-2 file:text-sm file:font-medium file:text-primary hover:file:bg-border disabled:opacity-60" />
      </label>
      <label className="grid gap-2 text-sm font-medium">
        Domain
        <select aria-label="Handbook domain" value={domainId} onChange={(event) => setDomainId(event.target.value)} disabled={!canManageSource || domains.length === 0} className="rounded border border-border bg-white px-3 py-2 font-normal outline-none focus:border-primary disabled:bg-accent">
        ...
        <button type="button" onClick={() => void submit()} disabled={!canManageSource || !selectedFile || !domainId || uploading} className="inline-flex h-10 items-center justify-center gap-2 rounded bg-primary px-4 text-sm font-medium text-white hover:bg-primary/90 disabled:opacity-40">
