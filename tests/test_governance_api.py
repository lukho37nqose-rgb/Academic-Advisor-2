...
def _metadata_steward() -> UserIdentity:
    return UserIdentity(
        tenant_id="tenant_demo_uni",
        role=Role.METADATA_STEWARD,
        user_id="steward_1",
        domain_ids=["dom_curr_2026"],
    )


def _policy_owner() -> UserIdentity:
    return UserIdentity(
        tenant_id="tenant_demo_uni",
        role=Role.POLICY_OWNER,
        user_id="policy_owner_1",
        domain_ids=["dom_curr_2026"],
    )
...
def test_draft_policy_rejects_uncompilable_payload_before_persistence():
    app.dependency_overrides[get_current_user] = _rule_author
...
def test_quick_edit_applies_metadata_overlay_and_audit_log(tmp_path):
    ...
    app.dependency_overrides[get_current_user] = _metadata_steward
...
def test_permission_matrix_exposes_role_boundaries():
    ...
    body = response.json()
    assert body["current_role"] == "metadata_steward"
    course_policy = next(
    ...
    steward_row = next(row for row in body["matrix"] if row["role"] == "metadata_steward")
    ...
    async def _exercise_register() -> None:
        async with session_factory() as session:
            ...
            assert await repository.has_open_ambiguities(
                tenant_id="tenant_demo_uni", domain_id="dom_curr_2026"
            )
...
def test_policy_ambiguity_api_uses_role_and_domain_scoped_workflow(tmp_path):
    ...
    try:
        ...
        app.dependency_overrides[get_current_user] = _policy_owner
